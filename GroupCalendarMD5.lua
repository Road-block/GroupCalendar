...
function GroupCalendar._MD5:DigestString(pString)
	local vStringLen = strlen(pString)
...
function GroupCalendar._MD5.swap(w)
	return GroupCalendar._MD5.StringToBE4(GroupCalendar._MD5.LEToString(w))
end
...
function GroupCalendar._MD5:Verify()
	...
	GroupCalendar:DebugMessage("MD5: All tests passed")
end
...
function GroupCalendar._MD5:CheckPerformance()
	...
	for _, vSize in ipairs(sizes) do
		...
		GroupCalendar:NoteMessage(format('%6d bytes: %4d ms', vSize, vElapsed))
	end
end
