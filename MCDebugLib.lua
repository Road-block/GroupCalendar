...
if not MCDebugLib or MCDebugLib.Version < gMCDebugLib_Version then
	...
	function MCDebugLib:Initialize()
		...
		self:SetScript("OnEvent", function() 
			if not MCDebugLib.DebugFrame then
				this:FindDebugFrame()
					end
				end)
	...
	function MCDebugLib:AddDebugMessage(pMessage, ...)
		...
		if false then -- set to false to diagnose debug message problems
			if arg.n > 0 and type(arg[1]) == "table" then
				self.DebugFrame:AddMessage(MCDebugLib.FormatNamed(pMessage, unpack(arg)))
			else
				self.DebugFrame:AddMessage(string.format(pMessage, unpack(arg)))
			end
		else
			local vSucceeded, vMessage
			
			if arg.n > 0 and type(arg[1]) == "table" then
				vSucceeded, vMessage = pcall(MCDebugLib.FormatNamed, pMessage, unpack(arg))
			else
				vSucceeded, vMessage = pcall(string.format, pMessage, unpack(arg))
			end
	...
	function MCDebugLib:DebugMessage(pMessage, ...)
		if self._AddOnName then
			MCDebugLib:DebugMessage(self._DebugColorCode.."["..self._AddOnName.."] "..FONT_COLOR_CODE_CLOSE..pMessage, unpack(arg))
		else
			MCDebugLib:AddDebugMessage(NORMAL_FONT_COLOR_CODE.."[DEBUG] "..HIGHLIGHT_FONT_COLOR_CODE..pMessage..FONT_COLOR_CODE_CLOSE, unpack(arg))
		end
	end

	function MCDebugLib:TestMessage(pMessage, ...)
		if self._AddOnName then
			MCDebugLib:TestMessage(self._DebugColorCode.."["..self._AddOnName.."] "..FONT_COLOR_CODE_CLOSE..pMessage, unpack(arg))
		else
			self:AddDebugMessage(GREEN_FONT_COLOR_CODE.."[   TEST] "..HIGHLIGHT_FONT_COLOR_CODE..pMessage..FONT_COLOR_CODE_CLOSE, unpack(arg))
		end
	end
	...
	function MCDebugLib:ErrorMessage(pMessage, ...)
		...
		if self._AddOnName then
			MCDebugLib:ErrorMessage(self._DebugColorCode.."["..self._AddOnName.."] "..vErrorMessageFontColorCode..pMessage..FONT_COLOR_CODE_CLOSE, unpack(arg))
		else
			local vFormat = RED_FONT_COLOR_CODE.."[ERROR] "..vErrorMessageFontColorCode..pMessage..FONT_COLOR_CODE_CLOSE
			
			if self.DebugFrame then
				self:AddDebugMessage(vFormat, unpack(arg))
			end
			
			if arg.n > 0 and type(arg[1]) == "table" then
				DEFAULT_CHAT_FRAME:AddMessage(MCDebugLib.FormatNamed(vFormat, unpack(arg)))
			else
				DEFAULT_CHAT_FRAME:AddMessage(string.format(vFormat, unpack(arg)))
			end
		end
	end
	...
	function MCDebugLib:NoteMessage(pMessage, ...)
		if self._AddOnName then
			MCDebugLib:NoteMessage(self._DebugColorCode.."["..self._AddOnName.."] "..FONT_COLOR_CODE_CLOSE..pMessage, unpack(arg))
		elseif arg.n > 0 and type(arg[1]) == "table" then
			DEFAULT_CHAT_FRAME:AddMessage(MCDebugLib.FormatNamed(NORMAL_FONT_COLOR_CODE..pMessage..FONT_COLOR_CODE_CLOSE, unpack(arg)))
		else
			DEFAULT_CHAT_FRAME:AddMessage(string.format(NORMAL_FONT_COLOR_CODE..pMessage..FONT_COLOR_CODE_CLOSE, unpack(arg)))
		end
	end
	
	function MCDebugLib:DumpArray(...) self:DebugTable(unpack(arg)) end
	...
	function MCDebugLib:ShowCallStack(...) return MCDebugLib:DebugStack(unpack(arg)) end -- Obsolete
	...
	function MCDebugLib:GetCallStack(pPrefix, pDepth, pDepthOffset)
		...
		for vMessageLine in string.gfind(vStackString, "(.-)\n") do
